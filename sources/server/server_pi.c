...
/*   Updated: 2018/11/05 16:26:08 by kcosta           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		command_handler_(int client, size_t len, char **argv)
{
	...
	{
		if (g_cmd_hash[i].cmd_len == len && !ft_strcmp(g_cmd_hash[i].cmd, argv[0]))
		{
			if (replace_path(&argv) && i != 3)
			{
				msg = ft_strjoin(g_cmd_hash[i].cmd, ": No such file or directory.");
